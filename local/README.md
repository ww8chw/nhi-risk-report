# 安安診所 · 健檢風險評估報告（本地版）

完全本地端跑、自動透過國健署「科學算病館」算 5 種慢性病 10 年風險。

## 安裝（一次）

1. 把整個 `local/` 資料夾複製到診所電腦（建議放桌面）
2. 第一次雙擊 `啟動.command`（Mac）或 `啟動.bat`（Windows）
3. Mac 第一次會跳「無法開啟」→「系統設定 → 隱私權與安全性」按「仍要打開」
4. 終端機自動裝 Bun（約 30 秒，僅這一次）→ 啟動代理 → 自動用 Chrome 開報告

## 平常使用

### 方式 A：直接從 Clinic Hub 點

1. 雙擊 `啟動.command` 啟動代理（終端機視窗會留著，別關）
2. 在 Clinic Hub 點「健檢報告產生器（本機）」卡片 → Chrome 新分頁開到 `http://localhost:7777`
3. 填表 → 「產生報告」→ 「列印 / 存 PDF」

### 方式 B：直接雙擊 .command（不經 Clinic Hub）

1. 雙擊 `啟動.command` → 自動開瀏覽器到 `http://localhost:7777`
2. 同樣填表 → 產生報告 → 列印

### 結束

關閉終端機視窗即停止代理。

## 列印設定

- 紙張：**A5**
- 方向：**橫向**

Chrome 列印對話框會自動帶入（@page CSS），但人員務必再次確認。

## 為什麼要本地跑

國健署 API（`cdrc.hpa.gov.tw`）在 TCP 層拒絕非台灣 IP 的連線。海外雲端 server（Railway us-west2 / Vercel 等）都是 `UND_ERR_CONNECT_TIMEOUT`。從台灣本地電腦直接打沒問題。

## 檔案結構

```
local/
├── 啟動.command    # Mac 雙擊啟動腳本
├── 啟動.bat        # Windows 雙擊啟動腳本
├── proxy.ts        # Bun 寫的本地代理（port 7777）
│                   # 同時 serve report.html 與 /api/risk-assessment
├── report.html     # 單檔 SPA — 全部 UI
└── README.md
```

`report.html` 也可直接 file:// 開（離線備援），但「產生報告」要 proxy 在跑。

## 疑難排解

**「啟動.command」無法執行 / 顯示「未驗證的開發者」**
- 系統設定 → 隱私權與安全性 → 滑到底有「仍要打開」按鈕

**Port 7777 已被佔用**
- 之前的代理沒關乾淨。重啟電腦最簡單。

**Clinic Hub 點「健檢報告產生器（本機）」沒反應 / 連線錯誤**
- 確認 `啟動.command` 還開著、終端機顯示「✓ 代理運作中」
- 沒有就再雙擊 `啟動.command`

**產生報告時所有疾病都「請求逾時」**
- 確認電腦能上網
- 確認沒掛 VPN（VPN 出口若是海外 IP 會被擋）

## 隱私

所有病人資料只在診所這台電腦的記憶體與瀏覽器 session 中。
代理不存任何資料到硬碟、不送任何資料到第三方（除了把計算用的數值送到國健署官方 API）。
關閉終端機視窗後資料即消失。
